...
def discover_infrastructure() -> str:
    ...
    try:
        ...
        # Since vuln_service uses network_mode: service:firegex, they share localhost.
        # But for the WAF configuration, we usually use the container IP or localhost depending on how the proxy logic works.
        # Firegex NFProxy forwards to ip_int. If mapped within same container, 127.0.0.1 is safer?
        # BUT Firegex IP logic retrieves the bridge IP.
        firegex_ip = ""
        ...
        # If no IP found (host mode?), fallback
        if not firegex_ip: firegex_ip = "127.0.0.1"
...
def setup_firegex_service(service_name: str, target_ip: str, port: int) -> str:
    ...
    if regex_id:
        ...
        s.post(f"{WAF_BASE_URL}/api/nfregex/services/{regex_id}/start")
        output.append("Firewall Service Started.")
        pass
